...
$scope.removemovie=function removemovie(catelog){
	if($scope.hasPermission() && catelog.id){
		...
		if(canDelte){
			...
			/*$http.delete('http://localhost:8080/spring-rest/api/catelogs/'+catelog.id)
            .success(function ( headers, config) {
                $scope.success="Successfulky deleted";							
				}
            )
...
$scope.addOrUpdate = function addOrUpdate(){
	if($scope.header === 'Add' && $scope.hasPermission()){
		...
		/*$http.post('http://localhost:8080/spring-rest/api/catelogs/', config)
	...
	if($scope.header === 'Update' && $scope.hasPermission()){
		...
		/*$http.put('http://localhost:8080/spring-rest/api/catelogs/'+catelogid, config)
            .success(function (data, status, headers, config) {
                $scope.Ruser = data;
